...
//This is a component that will be used in this scope only
function StatCard(props){
  const { lg, title, children } = props
  return(
    <div className={"card stat-card " + (lg ? " col-span-2" : "")}>
      <h4>{title}</h4>
      {children} {/**THis children is received from the <StatCard>"Children content</StatCard>" */}
    </div>
  )
}
...
export default function Stats(){
  ...
  const stats = calculateCoffeeStats(globalData)//this stats are coming from index.js
      ...
      <div className="stats-grid">
        <StatCard lg title="Active Caffeine Level">
          <div className="status">
            <p><span className="stat-text">{caffeineLevel}</span></p>
        ...
        <StatCard title="Avg # of Coffees">
          <p><span className="stat-text">{stats.average_coffees}mg</span></p>
        </StatCard>
